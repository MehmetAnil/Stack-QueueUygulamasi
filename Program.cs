...
namespace Proje2A2
{
    class Program
    {
        ...
        static void ÇözümMetodu(object[] alan, int[] çukurBoyutları, List<Canlı> karıncaListe,List<Canlı> arıListe, List<Stack> tListe) //arı parametresi
        {
                ...
                for (int z = 0; z < tListe.Count; z++)
                {
                    if (alan[i] != null)
                    {
                        if (alan[i].Equals(tListe[z]))
                        {
                            ...
                            //tListe.RemoveAt(z);
                            Console.WriteLine("ÇIKIŞ");
                            for (int a = karıncaListe.Count - 1; 0 <= a; a--)
                            {
                                if (karıncaListe[a] != null)
                                    Console.Write(karıncaListe[a].Ad + (a > 0 && karıncaListe[a - 1] != null ? "-" : ""));
                            }
                        }
                    }
                }
                ...
                for (int j = 0; j < çukurBoyutları.Length; j++)
                {
                    ...
                    if (alan[i] != null && alan[i].GetType().IsValueType)
                    {
                        if (çukurBoyutları[j] != 0 && ((int)alan[i]) == çukurBoyutları[j])
                        {
                            ...
                            //çukurBoyutları[j] = 0;
                            Console.WriteLine("ÇIKIŞ");
                            for (int z = arıListe.Count - 1; 0 <= z; z--)
                            {
                                if (arıListe[z] != null)
                                    Console.Write(arıListe[z].Ad + (z > 0 && arıListe[z - 1] != null ? "-" : ""));
                            }
                        }
                    }
                }
                for (int z = 0; z < tListe.Count; z++)
                {
                    if (alan[i] != null)
                    {
                        if (alan[i].Equals(tListe[z]))
                        {
                            ...
                            //tListe.RemoveAt(z);
                            for (int a = arıListe.Count - 1; 0 <= a; a--)
                            {
                                if (arıListe[a] != null)
                                    Console.Write(arıListe[a].Ad + (a > 0 && arıListe[a - 1] != null ? "-" : ""));
                            }
                        }
                    }
                }
            ...
            Console.WriteLine();
            Console.WriteLine("----VARIŞ NOKTASI SIRALAMA----");
            if (karıncaListe.Count > arıListe.Count)
            {
                for (int i = karıncaListe.Count - 1; 0 <= i; i--)
                {
                    Console.Write(karıncaListe[i].Ad + (i > 0 && karıncaListe[i - 1] != null ? "-" : ""));
            ...
            {
                for (int i = arıListe.Count - 1; 0 <= i; i--)
                {
                    Console.Write(arıListe[i].Ad + (i > 0 && arıListe[i - 1] != null ? "-" : ""));
                }
            }
        ...
        static void TuzakDüş(List<Canlı> karıncaListe,bool islem) //Can değerleri eklendiğinde bool kontrolü parametre olarak verilebilir.
        {
            ...
            if (karıncaListe[0].Can == 1)
            {
                if (islem)
                {
                    Console.WriteLine("\nTuzak Giriş, {0} Canlısı Öldü.", karıncaListe[0].Ad);
                }
                karıncaListe.RemoveAt(0);
